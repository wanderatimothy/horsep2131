...
if(count($data['custom_property_fields'])):
...
$rows = [];
$counter = 1;
for($i = 0; $i <count($data['custom_property_fields']); $i++):
    $rows['row_'.$counter][] = $data["custom_property_fields"][$i];
    if(($i+1) % 3 == 0){
        $counter ++;
    }
...
foreach($rows as $rowIndex => $rowValue):
    ...
    for($x=0; $x < count($rowValue); $x++):
        ...
        <div class="form-group col-g-4 col-md-4 col-sm-12">
            ...
            <input type="<?=$rowValue[$x]->type?>" class="form-control" name="<?=strtolower(str_replace(' ','_', $rowValue[$x]->name))?>"  >
        </div>
